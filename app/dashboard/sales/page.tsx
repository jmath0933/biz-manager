...
import { Calendar, TrendingDown, FileText, Package, TrendingUp } from "lucide-react";
...
export default function SalesPage() {
  ...
  const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
      ...
      const newUrl = `/dashboard/sales?${params.toString()}`;
  ...
  const fetchSales = async (start: string, end: string) => {
    setLoading(true);
    try {
      const url = `/api/sales?start=${start}&end=${end}`;
      ...
      const validSales = (data.sales || data || []).filter(
        (p: Sale) => p.fileUrl && p.fileUrl.trim() !== ""
      );
      
      console.log("📊 조회된 매입 데이터:", validSales.length, "건");
      setSales(validSales);
    } catch (error) {
  ...
  // 총합 계산
  const totalAmount = sales.reduce((sum, p) => sum + (p.totalAmount || 0), 0);
  const count = sales.length;

  // 월별 그룹화
  const groupedByMonth = sales.reduce((acc, p) => {
    ...
    year: string;
    month: string;
    items: (Sale & { day: string; formattedDate: string })[];
    total: number;
  }>);
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {data.items.map((p, idx) => {
                        ...
                        return (
                          <tr
                            key={p.id}
                            onClick={() => router.push(`/dashboard/sales/${p.id}`)}
